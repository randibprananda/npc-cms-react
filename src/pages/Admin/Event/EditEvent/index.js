...
import { useGetEventById } from "../../../../hooks/fetch/event/useGetEventById";
import { usePutEvent } from "../../../../hooks/fetch/event/usePutEvent";
...
const eventSchema = z
  ...
  .refine(
    (data) => {
      // Memeriksa apakah tanggal "Opening" lebih besar dari tanggal "Closing"
      const openingDate = new Date(data.opening);
      const closingDate = new Date(data.closing);
      return openingDate <= closingDate;
    },
    {
      message:
        "Tanggal Opening harus kurang dari atau sama dengan tanggal Closing",
      path: ["opening"], // Anda dapat mengganti ini sesuai dengan path yang sesuai dalam skema Anda
    }
  );
...
const EditEvent = () => {
  ...
  const { data: dataGetEventById } = useGetEventById(eventId);

  console.log(dataGetEventById);

  const { mutate: updateEvent } = usePutEvent({
    onSuccess: () => {
  ...
  const imagePath =
 process.env.REACT_APP_MODE == "dev" ? process.env.REACT_APP_API_IMAGE_URL_STAGING : process.env.REACT_APP_API_IMAGE_URL_PROD  + dataGetEventById?.image;
  const [image, setImage] = useState("");

  useEffect(() => {
    if (dataGetEventById !== undefined) {
      setImage(imagePath);
    }
  });

  const initialValues = {
    title: dataGetEventById?.title,
    opening: moment(dataGetEventById?.opening).format("YYYY-MM-DD"),
    closing: moment(dataGetEventById?.closing).format("YYYY-MM-DD"),
    location: dataGetEventById?.location,
    count: dataGetEventById?.count_down_time,
    image: dataGetEventById?.image,
  };
        ...
        <div className="w-screen -my-2 overflow-hidden">
          ...
          <div className="px-8 py-[70px]">
            <div className="py-[33px] px-[34px] shadow-card rounded-[10px]">
              <h1 className="mb-8 text-xl font-bold">Edit Event</h1>
              <div className="mb-4">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      reader.onloadend = () => {
                        ...
                        console.log(reader.result, "fileUploadede");
